...
class ImageToPyTorch(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ob = np.swapaxes(observation, 2, 0)
...
def phi(ob):
    ...
    X = torch.zeros(ob.shape[0],res,res)
    for frame in range(ob.shape[0]):
...
def wrap_pytorch(env, mode='train'):
    return ImageToPyTorch(env,mode=mode)
